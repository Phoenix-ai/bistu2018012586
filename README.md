...
Deep learning has been widely used in computer vision tasks such as image classification and object detection. However, in practical application scenarios, due to differences in lighting conditions, filter styles, etc. between the actual image and the image used for training the model, the prediction performance of the model in the actual scenario is often significantly reduced. Based on the style transfer technology, the style of the training set image is transferred to the actual image style, and the label label is kept unchanged, and a labeled training set with a style close to the specific actual image set can be generated. Training the model based on this training set will help It is used to improve the prediction performance of the model in specific real-world scenarios. This paper studies a data augmentation method based on style transfer, and evaluates its effect on model prediction performance on image classification tasks through experiments. Specifically, the model is trained using five types of data: data before style transfer, data after style transfer, new style data, data before style transfer enhanced during training, and data after style transfer enhanced during training. Based on the five trained models, make predictions on the new style test set and compare their effects. The results show that the data augmentation method based on style transfer can improve the performance of the model better than the traditional data augmentation method, and the effect of using these two data augmentation methods at the same time is better.
